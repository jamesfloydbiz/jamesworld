...
// Posts to leave off the writing page entirely (e.g. "On Abundance" — the
// handwritten Abundance poem already lives in the Poems grid).
const EXCLUDED_TITLES = ['on abundance'];
...
function bakeWritingPage(posts) {
  ...
  posts = posts.filter((p) => !EXCLUDED_TITLES.includes((p.title || '').trim().toLowerCase()));
