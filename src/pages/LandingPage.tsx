...
const LandingPage = () => {
  ...
  const handleLetterClick = () => {
    ...
    const rect = letterBtnRef.current?.getBoundingClientRect();
    if (!rect) return;
    setTransition({ type: 'letter', rect });
  ...
  const handlePortfolioClick = () => {
    ...
    if (!rect) return;
    setTransition({ type: 'portfolio', rect });
    setTimeout(() => navigate('/portfolio'), 850);
  };

  const fading = !!transition;
  ...
  return (
    <div
      ...
      {/* Content — fades out when transitioning */}
      <motion.div
        className="relative z-20 flex flex-col items-center text-center px-6 max-w-lg"
        variants={stagger}
        initial="hidden"
        animate={fading ? { opacity: 0, transition: { duration: 0.3 } } : 'show'}>
